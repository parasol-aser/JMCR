...
import edu.tamu.aser.mcr.graph.LockSetEngine;
...
import edu.tamu.aser.mcr.trace.ISyncNode;
import edu.tamu.aser.mcr.trace.JoinNode;
import edu.tamu.aser.mcr.trace.LockNode;
import edu.tamu.aser.mcr.trace.LockPair;
import edu.tamu.aser.mcr.trace.NotifyNode;
import edu.tamu.aser.mcr.trace.PropertyNode;
import edu.tamu.aser.mcr.trace.ReadNode;
import edu.tamu.aser.mcr.trace.StartNode;
...
public class ConstraintsBuildEngine
{
	...
	protected HashMap<AbstractNode,AbstractNode> partialOrderMap = new HashMap<AbstractNode,AbstractNode>();
	protected HashMap<String,Vector<LockPair>> lockPairsMap = new HashMap<String,Vector<LockPair>>();
	protected HashMap<Long,Vector<LockPair>> threadLockPairs = new HashMap<Long,Vector<LockPair>>();
	 ...
	 * @param cons
	 */
	public void declareVariables(StringBuilder cons){
...
//				if (!depNodes.contains(nodes.get(0))) {
//					continue;
//				}
		 ...
		 */
		Iterator<Vector<AbstractNode>> mapIt = mapthreadIdtoNode.values().iterator();  //key is thr_id, values are the trace in each thrd.
		while(mapIt.hasNext())
		{
...
//			if (!depNodes.contains(nodes.get(i))) {
//				continue;
//			}
		 ...
		 */
		if (Configuration.mode == "TSO") {
			Iterator<Vector<AbstractNode>> mapItw = mapthreadIdtoNode.values().iterator();  //key is thr_id, values are the trace in each thrd.
			while(mapItw.hasNext())
			{
				Vector<AbstractNode> nodes = mapItw.next();
...
//						if (depNodes.contains(nodes.get(j))) 
		 ...
		 */
		Iterator<Vector<AbstractNode>> mapItmem = mapthreadIdtoNode.values().iterator();
		while(mapItmem.hasNext()){
			Vector<AbstractNode> nodes = mapItmem.next();
			...
			for(int i=0; i<nodes.size(); i++){
				if(nodes.get(i) instanceof ReadNode || nodes.get(i) instanceof WriteNode){
					//memNodes.add(nodes.get(i));
					continue;
				}
				else{
					if (depNodes.contains(nodes.get(i))) {
						nonMemNodes.add(nodes.get(i));
					}
...
//			}
		 ...
		 */
		Iterator<Vector<AbstractNode>> mapItr = mapthreadIdtoNode.values().iterator();
		while(mapItr.hasNext()){
			Vector<AbstractNode> nodes = mapItr.next();
			
			/**
			 * Profile the read nodes
			 */
			//Vector<AbstractNode> readNodes = new Vector<AbstractNode>();
	...
	public void constructSyncConstraintsRMM(Trace trace, HashSet<AbstractNode> depNodes){
		...
		Iterator<Vector<ISyncNode>> mapIt = syncNodesMap.values().iterator(); //iterate the values
		while(mapIt.hasNext())
		{
			...
			{			
				...
				else if (node instanceof JoinNode)  //thread join and the last action of that thread happens in different threads
				{
					if (depNodes.contains(node)) {
						long tid = Long.valueOf(node.getAddr());	
						/**
						...
						if(Configuration.mode=="TSO" || Configuration.mode=="PSO"){
							...
							if (nodesSet != null) {
								for(int j=0; j<nodesSet.size(); j++){
									if (depNodes.contains(nodesSet.get(j))) {
										long nodeGID = nodesSet.get(j).getGID();
										String nodeVar = makeVariable(nodeGID);
		...
		//lock constraints
		for(Iterator<Vector<LockPair>> it2 = lockPairsMap.values().iterator(); it2.hasNext();){
			Vector<LockPair> LPs = it2.next();
			{
	 ...
	 * @return
	 */
	public Vector<ReadNode> realReadDeps (Vector<ReadNode> w1_readnodes, AbstractNode destNode){	
...
//			System.err.println("wrong");
		...
		Iterator<ReadNode> it = w1_readnodes.iterator();
		while (it.hasNext()){
			ReadNode node = it.next();
			...
			//source != dest??
			if (source != dest && Configuration.reachSDG.canReach(source, dest)){
	 ...
	 * @param depNodes
	 * @param readNodes
	 * @param otherVariableNodes
	 * @return
	 */
	public String constructDataValidityConstraints(Trace trace,
			...
			while(readNodes.size()>0) {
				...
				if(canMatchInit)
				{
					...
					if(initValue!=null&&rValue.equals(initValue)
						...
						for(int k=0;k<writenodes_other.size();k++)
						{
							WriteNode wnode3 = writenodes_other.get(k);
				...
				if(!canMatchInit)
				{
					...
					for(int j=0;j<writenodes_value_match.size();j++)
					{
						...
						for(int k=0;k<writenodes_other.size();k++)
						{
							WriteNode wnode2 = writenodes_other.get(k);
	...
	public void constructSyncConstraints(Trace trace, HashSet<AbstractNode> depNodes)
	{
		...
		for(Iterator<Entry<AbstractNode,AbstractNode>> it = 
				partialOrderMap.entrySet().iterator();it.hasNext();)
		{
			Entry<AbstractNode,AbstractNode> entry = it.next();
...
//		HashSet<AbstractNode> otherVariableNodes = new HashSet<AbstractNode>();	
			...
			AbstractNode rnode, AbstractNode wnode, Vector<AbstractNode> writenodes)
	{
		...
		for(int k=0;k<writenodes.size();k++)
		{
			AbstractNode wnode2 = writenodes.get(k);
			if(wnode!=wnode2
	...
	public Vector<ReadNode> getReadNodes(HashSet<AbstractNode> depNodes)
	{
		Vector<ReadNode> readnodes = new Vector<ReadNode>();
		//traverse depNodes to find all read nodes
		 
				for(Iterator<AbstractNode> it = depNodes.iterator();it.hasNext();)
				{
...
//				}
	...
	public void preprocess(Trace trace) {
		...
		//thread first node - last node
		Iterator<String> addrIt = syncNodesMap.keySet().iterator();
		while(addrIt.hasNext())
		{
			...
			{			
				...
				{//interpret waitnode as unlock-wait, the trace contains another lock node
