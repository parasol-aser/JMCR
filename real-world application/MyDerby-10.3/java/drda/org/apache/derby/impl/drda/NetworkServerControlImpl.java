...
//			}
		...
		throws Exception
	{
		...
		clientThread.start();//let's see what the client thread  doing?
        ...
        //oh, it takes so long a procedure to close the service
		...
		{
 			//interrupt any connection threads still active
		...
		synchronized (serverStartSync) {
			...
			// should still work correctly, just not as efficiently...))
				...
				try {
					...
					// start the server.
					Class.forName(CLOUDSCAPE_DRIVER).newInstance();//start the embedded driver, it starts two daemon threads: derby.antiGC and Time-0
