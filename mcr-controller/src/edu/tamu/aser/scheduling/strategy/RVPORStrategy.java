...
import edu.tamu.aser.ReExProperties;
import edu.tamu.aser.rvinstrumentation.RVGlobalStateForInstrumentation;
import edu.tamu.aser.rvinstrumentation.RVPORRunTime;
...
import edu.tamu.aser.RVCausalTrace;
...
public class RVPORStrategy extends SchedulingStrategy {
	...
	static {
		fullTrace = Boolean.parseBoolean(ReExProperties.getInstance()
				.getProperty(ReExProperties.RV_CAUSAL_FULL_TRACE, "true"));
		
        MAX_NUM_PREEMPTIONS = Integer.parseInt(ReExProperties.getInstance().getProperty(ReExProperties.PREEMPTION_BOUND_KEY, Integer.MAX_VALUE + ""));
        if (MAX_NUM_PREEMPTIONS == Integer.MAX_VALUE) {
            System.out.println("\nWARNING: no value specified for property \"" + ReExProperties.PREEMPTION_BOUND_KEY + "\"");
